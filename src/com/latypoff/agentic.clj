...
(defn- wrap-arity
  [defn-sym form source-path env arity]
  (let [[params & body] arity
        [prepost body] (if (and (map? (first body)) (next body))
...
(defmacro defn
  ...
  (let [file *file*
        source-path (try
                      (when-let [v (or (resolve '*source-path*)
                                       (ns-resolve 'clojure.core '*source-path*))]
                        (when (and (var? v) (bound? v))
                          (let [val (var-get v)]
                            (when (and (string? val)
                              ...
                              val))))
                      (catch Throwable _))
        path (or source-path file)
        parsed (apply parse-defn args)
        name-sym (:name parsed)
        defn-sym (if (namespace name-sym)
                   name-sym
                   (symbol (str (ns-name *ns*)) (name name-sym)))
        bodies (map #(wrap-arity defn-sym &form path &env %)
                    (:bodies parsed))]
    `(clojure.core/defn ~name-sym
